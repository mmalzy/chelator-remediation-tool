...
def predict_single(models, encoders, feature_info, params):
    ...
    for col in feature_info["feature_columns"]:
        if col == "chelator_encoded":
            chelator_val = params.get("chelator", "None")
            le = encoders["chelator"]
            if str(chelator_val) in le.classes_:
...
def find_best_recommendation(comparison_df):
    ...
    treated = comparison_df[comparison_df["Chelator"] != "nan"].copy()
    baseline = comparison_df[comparison_df["Chelator"] == "nan"].iloc[0]
