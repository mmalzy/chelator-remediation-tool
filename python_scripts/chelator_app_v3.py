"""
Chelator ML Remediation Tool — v3 (Professional Interface)
...
Usage:
    ...
    python3 -m streamlit run chelator_app_v3.py
"""
...
# ---------------------------------------------------------------------------
# Professional CSS — no emojis anywhere
# ---------------------------------------------------------------------------
st.markdown("""
<style>
/* ---- Import fonts ---- */
...
h1, h2, h3, h4 { font-family: 'Playfair Display', serif; color: #0f2b46; }
...
.stTabs [aria-selected="true"] {
    ...
    border-bottom: 3px solid #1a4a6e !important;
...
.card-excellent { background: #f0fdf4; border-color: #16a34a; }
.card-good      { background: #f0f9ff; border-color: #2563eb; }
.card-moderate  { background: #fffbeb; border-color: #d97706; }
.card-poor      { background: #fef2f2; border-color: #dc2626; }
...
.metric-box .value { font-size: 1.6rem; font-weight: 700; color: #0f2b46; }
.metric-box .label { font-size: 0.78rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.6px; }
...
# ---------------------------------------------------------------------------
# Predictions for all chelator × dose combinations  (+ No Treatment baseline)
# ---------------------------------------------------------------------------
...
def run_full_comparison(_ph, _pb, _cu, _zn, _cd, _doc, _ca, _mg, _na, _cl,
    ...
    rows.append({"Chelator": "No Treatment", "Dose (mg/L)": 0, **{f"{m} % Free": round(preds[m], 1) for m in METALS}})

    # All chelator × dose combos
