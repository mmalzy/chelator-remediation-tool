"""
Chelator ML Remediation Tool — v4.9 (Debugged, Scientifically Validated)
Rhode Island Coastal Soil Remediation Decision Support System

Changes from v4.8:
    - Fixed USDA texture classification (full 12-class triangle, mapped to 3 model bins)
    - Added pH extrapolation warnings (training range: 5.5-7.5)
    - Weighted best-overall recommendation by metal treatability
    - Aligned Ca/Mg and Na/Cl sliders to actual training data levels
    - Updated chelator ranking findings to match verified data
    - Fixed Pb superscript notation (Pb2+ not Pb+2)

Usage:
    cd /Users/mallorymalz/Documents/chelator_ml_project/python_scripts
    python3 -m streamlit run chelator_app_v4_9.py
"""
...
# ---------------------------------------------------------------------------
def classify_texture_usda(sand, silt, clay):
    """Full USDA soil texture classification (12 classes)."""
    if clay >= 40:
        if silt >= 40:
            return "Silty Clay"
        elif sand <= 45:
...
# Mapping from 12 USDA classes to 3 model texture bins
USDA_TO_MODEL_BIN = {
    "Sand": "Sand",
    "Loamy Sand": "Sand",
    "Sandy Loam": "Loam",
...
def classify_texture(sand, silt, clay):
    """Classify soil texture into Sand/Loam/Clay model bin via full USDA triangle."""
    usda_class = classify_texture_usda(sand, silt, clay)
    return USDA_TO_MODEL_BIN.get(usda_class, "Loam"), usda_class
...
with st.sidebar:
    ...
    if ph < 5.5 or ph > 7.5:
        st.warning("Model was trained on pH 5.5–7.5. Predictions outside this range are extrapolations and less reliable.")
    ...
    texture, usda_class = classify_texture(sand_pct, silt_pct, clay_pct)
    hfo, doc = texture_to_params(texture)
    st.caption(f"USDA Classification: **{usda_class}** (Model bin: {texture})")
    ...
    if is_coastal:
        na_val = st.select_slider("Salinity level",
                                  options=[500, 2000],
                                  value=500,
                                  format_func=lambda x: {500: "Moderate (500 mg/L Na)",
                                                         2000: "High (2000 mg/L Na)"}[x])
        # Use the Cl values from training data that pair with each Na level
        cl_val = {500: 700, 2000: 3000}[na_val]
    else:
        na_val = 100
        cl_val = 150
    ...
    ca_val = st.select_slider("Calcium/Magnesium competition",
                              options=[20, 100],
                              value=20,
                              format_func=lambda x: {20: "Low (20 mg/L Ca)",
                                                     100: "High (100 mg/L Ca)"}[x])
    mg_val = ca_val / 2.0
...
# Best overall chelator — weighted by each metal's responsiveness to treatment.
# Zn barely responds to chelation (mean 84% free), so down-weight it to avoid
# letting a metal that can't be improved dominate the recommendation.
treatment_rows = comparison_df[comparison_df["Chelator"] != "No Treatment"].copy()
metal_cols = [f"{m} % Free" for m in METALS]

# Weights based on metal treatability: higher weight = metal responds more to chelation
METAL_WEIGHTS = {"Pb": 0.30, "Cu": 0.35, "Zn": 0.10, "Cd": 0.25}
treatment_rows["weighted_free"] = sum(
    treatment_rows[f"{m} % Free"] * w for m, w in METAL_WEIGHTS.items()
)
best_idx = treatment_rows["weighted_free"].idxmin()
...
with tab_site:
    ...
    with c2:
        st.markdown(f"""
        <div class="metric-box"><div class="label">Texture</div><div class="value">{usda_class}</div><div class="label" style="margin-top:0.2rem;">Model bin: {texture}</div></div>
        """, unsafe_allow_html=True)
    with c3:
        st.markdown(f"""
...
with tab_about:
    ...
    <div class="about-section">
        ...
        <ul>
            ...
            <li>High ionic strength (coastal salinity) can reduce free metal through chloride complexation — especially for Cd.</li>
            <li>Optimal chelator varies by metal: Citrate for Pb, NTA for Cu/Zn/Cd. EDTA is not always the best choice despite being the industry standard.</li>
            <li>Choosing the wrong chelator or underdosing can increase free Pb relative to no treatment (competitive desorption effect).</li>
        </ul>
    </div>
    ...
    <div class="about-section">
        ...
        <ul>
            ...
            <li>Predictions are most reliable within the training pH range (5.5&ndash;7.5) and trained parameter levels.</li>
            <li>The &ldquo;best overall&rdquo; recommendation weights metals by treatability (Cu &gt; Pb &gt; Cd &gt; Zn). Per-metal best chelators are also shown.</li>
